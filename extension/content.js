...
(function () {
        ...
        const replInput = $("<input type=\"text\" placeholder=\"REPL: alert('Happy Hacking!')\" >");
        const replDiv = $("<div id=\"replDiv\" class=\"replDiv\"></div>");
        replDiv.append(replInput)
        ...
        let replInputTextChangeKeyDown$ = rxjs.fromEvent(replInput, 'keydown')
            .pipe(
                ...
                rxjs.filter(res => res.keyCode === 13),
        ...
        let replInputTextChangeKeyUp$ = rxjs.fromEvent(replInput, 'keyup')
            .pipe(
                rxjs.map((event) => {
                    return event;
                }),
